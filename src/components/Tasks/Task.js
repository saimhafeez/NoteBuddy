...
import { createTodoTask, getTodoById, updateTodoTask } from "../../db/ToDoOperations";
...
function Task({ data }) {
    ...
    const { setTaskSymbolChanged, taskSymbolChanged, taskMarkChanged, setTaskMarkChanged } = useUiRefresh()
    const [_data, set_data] = useState(null)

    useEffect(() => {
        // console.log('id -->', data.id)
        getTodoById(data.id).then((results) => {
            set_data(results)
        }).catch((err) => {
            console.log('err -->', err)
        })
    ...
    const markToDo = () => {
        ...
        updateTodoTask(data.id, JSON.stringify(_data.task), !_data.isCompleted, completedOn, JSON.stringify(_data.symbol)).then(r => {
            ...
            if (_data.task.schedule && _data.task.schedule.repeat) {
                ...
                _task.schedule.date = GetNextRepeatingDate(_data.task.schedule.date, _data.task.schedule.repeat.toLocaleLowerCase())

                createTodoTask(JSON.stringify(_task), false, '', JSON.stringify(TaskSymbols.default)).then((r) => {
    ...
    const taskRepeatStyle = () => {
        ...
        return _data.task && !_data.isCompleted && prev_date.isBefore(current_date) ? currentTheme.linkColorDanger : currentTheme.TextColor
    }
    ...
    return (
        _data && <View style={[styles.container, { backgroundColor: currentTheme.TaskItemContainerBackgroundColor }]}>

            <TouchableOpacity onPress={markToDo}>
                <MyIcon iconPack='MaterialIcons'
                    name={_data.isCompleted ? 'check-circle' : 'radio-button-unchecked'}
                    size={26} color={currentTheme.IconColor}
                />
            </TouchableOpacity>
            ...
            <View style={{ flex: 1, marginLeft: 10, marginRight: 10 }}>
                ...
                {_data.task.schedule.repeat &&
                    <View style={[myStyles.flexRowWithGap, { alignItems: 'center', gap: 0 }]}>
                        <MyText
                            // style={ TwentyFourHoursFormat(_data.task.schedule) && {color: currentTheme.linkColorDanger, fontSize: 11}}
                            style={{ fontWeight: 'bold', opacity: 0.34, fontSize: 11, color: taskRepeatStyle() }}
                        >
                            {_data.task.schedule.date.toLocaleLowerCase().replaceAll('-', '/')}
                        </MyText>
                        {
                            !_data.completedOn && <MyIcon iconPack='MaterialCommunityIcons' name='repeat' size={16} />
                        }
                    </View>}
            </View>
            ...
            <TouchableOpacity onPress={() => {
                ...
                <MyIcon
                    ...
                    size={26}
                    color={_data.symbol.name === 'flag-outline' ? currentTheme.IconColor : _data.symbol.color}
                />
            </TouchableOpacity>
