...
function AddNewTask({ data }) {
    ...
    const { openModal, closeModal, getModalData, stackOpenModal, setDataForModal } = useModal()
                ...
                <View style={{ flex: 1, gap: 5 }}>
                    ...
                    />
                    {
                        Object.keys(task.subTasks).map((taskKey, index) => {
                            return <View style={[myStyles.flexRowWithGap, { alignItems: 'center' }]} key={index}>
                                <TouchableOpacity
                                    onPress={() => {
                                ...
                                >
                                    <MyIcon name='minuscircle' iconPack='AntDesign' color={currentTheme.linkColorDanger} size={24} />
                                </TouchableOpacity>
                                <TextInput
                                    style={[myStyles.button, { flex: 1, borderColor: currentTheme.TextFieldBackground, color: currentTheme.TextColor, backgroundColor: currentTheme.TextFieldBackground }]}
