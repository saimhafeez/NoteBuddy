...
import { FlatList, TouchableOpacity, View, Button, TextInput } from "react-native";
...
function AddAudioNote() {
  ...
  async function getAllAudioFiles() {
    try {
      ...
      const audioFiles = files.filter((file) => file.startsWith('audio_') && file.endsWith('.m4a'));

      // Return an array of file URIs
      return audioFiles.map((fileName) => `${FileSystem.documentDirectory}${fileName}`);
    } catch (error) {
  ...
  function startPlayback() {
    console.log('keys', Object.keys(recordingData.sound), recordingData.sound.getStatusAsync);
  ...
  return (
    <View style={myStyles.flexColumnWithGap}>
      ...
      {
        timer &&
        <View style={[myStyles.flexRowWithGap, { justifyContent: 'space-between' }]}>
          <MyText>Duration: {getDurationFormatted(timer)}</MyText>

          <TouchableOpacity onPress={startPlayback}>
            <MyIcon iconPack='FontAwesome' name={audioPlayback ? 'pause-circle' : 'play-circle'} color={isRecording ? currentTheme.linkColorDanger : currentTheme.IconColor} />
            {/* <MyText>{currentRecording ? 'Play' : 'Pause'}</MyText> */}
          </TouchableOpacity>
        </View>
      }

      <View style={[myStyles.flexRowWithGap, myStyles.input, {
        width: '100%',
        flex: 1,
        alignItems: 'center',
        minHeight: 160 * 0.4
        ...
        <TouchableOpacity onPress={currentRecording ? stopRecording : startRecording}>
          <MyIcon iconPack='MaterialCommunityIcons' name='record-circle-outline' color={isRecording ? currentTheme.linkColorDanger : currentTheme.IconColor} />
      ...
      <View style={myStyles.flexRowWithGap} >
        ...
        <TouchableOpacity
          style={[myStyles.button, { justifyContent: 'center', backgroundColor: currentTheme.linkColor }]}
          onPress={() => { }}>
