...
function TaskSchedule({ data }) {
    ...
    const { stackCloseModalAndDispose, setDataForModal, getModalData, openModal, stackCloseModal } = useModal()
    ...
    const scheduleTask = () => {
        ...
        mData ? setDataForModal('AddNewTask', {
            ...mData,
            schedule
        })
    ...
    return (
        <ScrollView contentContainerStyle={{ gap: 10 }}>
            ...
            <View style={[myStyles.flexRowWithGap, { flexWrap: 'wrap' }]}>
                ...
                <TouchableOpacity
                    style={[myStyles.button, schedule.date === '' && { backgroundColor: currentTheme.linkColor }]}
                    onPress={() => setSchedule(prevState => ({ ...prevState, date: '' }))}
                >
                    <MyText style={schedule.date === '' && { color: currentTheme.Background, fontWeight: 'bold' }}>No Date</MyText>
                </TouchableOpacity>

                <TouchableOpacity
                    style={[myStyles.button, schedule.date === getDateFromNow(0) && { backgroundColor: currentTheme.linkColor }]}
                    onPress={() => setSchedule(prevState => ({ ...prevState, date: getDateFromNow(0) }))}
                >
                    <MyText style={schedule.date === getDateFromNow(0) && { color: currentTheme.Background, fontWeight: 'bold' }}>Today</MyText>
                </TouchableOpacity>

                <TouchableOpacity
                    style={[myStyles.button, schedule.date === getDateFromNow(1) && { backgroundColor: currentTheme.linkColor }]}
                    onPress={() => setSchedule(prevState => ({ ...prevState, date: getDateFromNow(1) }))}
                >
                    <MyText style={schedule.date === getDateFromNow(1) && { color: currentTheme.Background, fontWeight: 'bold' }}>Tomorrow</MyText>
                </TouchableOpacity>

                <TouchableOpacity
                    style={[myStyles.button, schedule.date === getDateFromNow(3) && { backgroundColor: currentTheme.linkColor }]}
                    onPress={() => setSchedule(prevState => ({ ...prevState, date: getDateFromNow(3) }))}
                >
                    <MyText style={schedule.date === getDateFromNow(3) && { color: currentTheme.Background, fontWeight: 'bold' }}>3 Days Later</MyText>
                </TouchableOpacity>

                <TouchableOpacity
                    style={[myStyles.button, schedule.date === getNearestSundayDate() && { backgroundColor: currentTheme.linkColor }]}
                    onPress={() => setSchedule(prevState => ({ ...prevState, date: getNearestSundayDate() }))}
                >
                    <MyText style={schedule.date === getNearestSundayDate() && { color: currentTheme.Background, fontWeight: 'bold' }}>This Sunday</MyText>
                </TouchableOpacity>
            ...
            <View style={myStyles.flexColumnWithGap}>
                <TouchableOpacity
                    ...
                    style={[myStyles.flexRowWithGap, { justifyContent: 'space-between' }, myStyles.button]}
                >
                    <View style={myStyles.flexRowWithGap}>
                        <MyIcon iconPack='MaterialIcons' name='timer' color={currentTheme.linkColor} size={26} />
                        <MyText>Time</MyText>
                    </View>
                ...
                >
                    <View style={myStyles.flexRowWithGap}>
                        <MyIcon iconPack='MaterialIcons' name='repeat' color={currentTheme.linkColor} size={26} />
...
function getNearestSundayDate() {
    ...
    const nearestSundayDate = new Date(currentDate.getTime() + (daysUntilSunday * 24 * 60 * 60 * 1000));
