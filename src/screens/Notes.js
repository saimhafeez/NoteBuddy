import { Text, View, Button, ScrollView, Platform, KeyboardAvoidingView, SafeAreaView, TextInput, TouchableOpacity, Pressable } from "react-native";
...
function Notes({ navigation }) {
    ...
    const deleteNoteModal = () => {
        return <View
            ...
            <View style={[selectedNoteForDeletion.password && myStyles.input, myStyles.flexRowWithGap, { alignItems: 'center', justifyContent: 'center' }]}>
                {selectedNoteForDeletion.password && <TextInput
                    // value={}
                ...
                >
                    <View style={[myStyles.flexRowWithGap, { alignItems: 'center', justifyContent: 'center' }]}>
                        ...
                        {!selectedNoteForDeletion.password && <MyText>Delete Note ? </MyText>}

                        <MyIcon iconPack='MaterialCommunityIcons' name='delete' color={currentTheme.linkColorDanger} size={35} />
                    </View>
                </TouchableOpacity>
        ...
        <View style={{ flex: 1, backgroundColor: currentTheme.Background }}>
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => {
                    return <TouchableOpacity onLongPress={() => deleteNoteDialog(item)} onPress={() => navigateToNote(item)} style={[myStyles.input, myStyles.flexRowWithGap, { justifyContent: 'space-between', alignItems: 'center' }]}>
                        <View style={[myStyles.flexRowWithGap, { alignItems: 'center' }]}>
                            {item.password && <MyIcon name='lock' iconPack='MaterialCommunityIcons' />}
                            <MyText style={{ fontWeight: 'bold' }}>{item.title}</MyText>
                        </View>
                        <MyText>{new Date(item.dateModified).toLocaleDateString()}</MyText>
                    </TouchableOpacity>
                }}
            />
